...
async def researcher(state: AgentState) -> dict:
    ...
    return {"retrieved_docs": docs, "messages": [response], "steps": state.get("steps", 0) + 1}
...
def writer(state: AgentState) -> dict:
    web_results = [m.content for m in (state.get("messages") or []) if isinstance(m, ToolMessage)]
